...
//	levels[level].markerLayer.setFilter(markerFilterFunction).addTo(map);		 
    ...
    map.getRoute = function(a, b, callback) {
      ...
      $.ajax({
	url: 'http://navi.indoor.io/navi/maps/28774eb97235facf369d56a8b4027500/route.jsonp?'+
   ...
   function search(mapid, query, callback) {
	if(!mapid) mapid = "07b23f7c752905e23891c9a319e617b2";//"6b269e5a7779cfdc1912edbef09cc624"; //"28774eb97235facf369d56a8b4027500";
            ...
            try {
                ...
                $.getJSON('http://navi.indoor.io/navi/maps/' + mapid +
                          '/search.jsonp?' + str +
                          '&callback=?')
  ...
  map.parseUTFGridData = function(data) {
	  ...
	  if(typeof data[d] == 'string' && data[d].length == 0) continue;
        ...
        var feature = {properties: properties, geometry: JSON.parse(data.geometry),
          levelIndex: data.levelIndex ? data.levelIndex : 2};
        if(feature.geometry.type == 'Polygon') {
...
/*	      geojson.index = {};
		...
		var feature = geojson.features[f];
		  ...
		  } else if(geojson.features[f].geometry.type == 'Point') {
		    geojson.features[f].properties.markerLocation = new L.indoor.latLng(geojson.features[f].geometry.coordinates[1],
										       geojson.features[f].geometry.coordinates[0],
										       data.name);
...
*/	       
	       if(false && map.getLevel() == data.name && markerFilterFunction) {
